...
// ── Fetch budget spending for current month ────────
...
// ── Render budget cards (budget page) ─────────────
export function renderBudgetCards(budgets, spending) {
  ...
  if (budgets.length === 0) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1; padding:3rem;">
        ...
        <div class="empty-state-title">Belum ada budget</div>
        <div class="empty-state-desc">Klik "+ Tambah Budget" untuk mulai mengatur anggaran kamu</div>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = budgets.map((b) => {
    ...
    return `
      <div class="budget-full-card animate-fade-in">
...
// ── Render budget overview (dashboard widget) ──────
  ...
  if (budgets.length === 0) {
    el.innerHTML = `
      <div class="empty-state" style="padding:1.5rem;">
        <div class="empty-state-icon">💰</div>
        <div class="empty-state-title" style="font-size:0.9rem;">Belum ada budget</div>
      </div>
    `;
    return;
  }
      ...
      <div class="budget-item">
        <div class="budget-item-header">
          <div class="budget-item-label">
            <span class="budget-item-emoji">${cat?.icon || '💰'}</span>
            ${cat?.name || 'Kategori'}
          </div>
          <div class="budget-item-amount">
            <span>${formatCurrency(spent)}</span> / ${formatCurrency(budget)}
          </div>
        </div>
        <div class="progress-bar">
          <div class="progress-fill ${fillClass}" style="width:${percent}%"></div>
